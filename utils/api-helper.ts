...
export class ApiHelper {
  ...
  constructor(request: APIRequestContext, baseURL?: string) {
    if (!request) {
      throw new Error(
        "ApiHelper: Request context is required but was undefined/null"
      );
    }
  ...
  async post(
    ...
    try {
      ...
      if (errorMessage.includes("Cannot read properties of undefined")) {
        throw new Error(
          `POST request failed for ${endpoint}: Request context is not properly initialized. This usually indicates a test setup issue. Original error: ${errorMessage}`
        );
      }
